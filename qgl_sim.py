...
class Simulation():
    ...
    def W(self):
        ...
        """
        return (sum([self.one_alive(k) for k in range(2,self.L-2)]) \
                +4*self.one_alive())*1./sqrt(self.L)
    ...
    for L in Llist:
        for dt in dtlist:
            for tspan in tspanlist:
                for IC in IClist:
                    IC = OrderedDict(IC)
                    mySim = Simulation(tasks,L,tspan,dt,IC,states_path = DATA_PATH,meas_path = DATA_PATH)
...
#main('nonFockTest',['t','n','nn','MI'],[7],[0.1],[10],[[('W',1./sqrt(2)),('d28',1./sqrt(2))],[('W',1./sqrt(2)),('d12',1./sqrt(2))]])
