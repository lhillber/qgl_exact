...
def plot_specgram(mydata,task,subtask,fignum=1,ax=111,yax_label='Frequency',title='title',NFFT=420):
    ...
    fig.add_subplot(ax)
    Pxx, freqs, bin= \
    mlab.specgram(np.array(time_series),window=mlab.window_none,Fs=int(1/(mydata['dt'])),NFFT=NFFT,noverlap=NFFT-1,pad_to=600)
    plt.pcolormesh(bin,freqs,Pxx,rasterized=True)
    cbar=plt.colorbar()
...
def time_average(mydata,task,subtask):
    ...
    #sd = qof.standard_deviation(time_series)
    sd = 1
    return avg,sd
...
def multipage(fname,figs=None, dpi=300): 
    pp = PdfPages(fname) 
    if figs is None:
...
def make_tave_sweep(batch_path,L,dt,tspan,IClist,task,subtask):
    ...
    for IC in IClist:
        ...
        ave,sd = time_average(mydata,task,subtask)
        avelist.append(ave)
        sdlist.append(sd)
    return avelist, sdlist
...
def plot_tave_sweep(batch_path,L,dt,tspan,IClist,task,subtask,thlist,fignum=1,ax=111,yax_label='Equilibrium val.',title='title'):
    avelist,sdlist = make_tave_sweep(batch_path,L,dt,tspan,IClist,task,subtask) 
    fig = plt.figure(fignum)
    fig.add_subplot(ax)
    plt.plot(thlist,avelist,'o')
    ...
    for L in Llist:
        for dt in dtlist:
           for tspan in tspanlist:
               ...
               plot_tave_sweep(batch_path,L,dt,tspan,IClist,'nn','ND',thlist,fignum=29,ax=111,yax_label=r'ND$_{\infth}$',title=r'g$_{ij}$ Equilibrium ND')
               ...
               multipage('../'+batch_path+'/plots/'+'equibvals')
               '''
               ...
               for IC in IClist:
                   ...
                   mydata = import_data(batch_path,L,dt,tspan,IC) 
                   time_plots(mydata)
                   multipage(sim_name(batch_path,L,dt,tspan,IC,subdir='plots')+'_plots.pdf')
                   plt.close()
